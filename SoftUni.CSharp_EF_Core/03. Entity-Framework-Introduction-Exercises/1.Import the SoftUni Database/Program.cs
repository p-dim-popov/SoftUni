...
namespace _1.Import_the_SoftUni_Database
{
    static class Program
    {
        static void Main(string[] args)
        {
            Console.Write("Warning!!! This will drop the database and recreate it! Continue? [y/N] ");
            var decision = Console.ReadLine();
            if (decision != "y" && decision != "Y")
                return;


            using (var conn = new SqlConnection("server=.;database=master;integrated security=true;"))
            {
                conn.Open();
                using (var command = conn.CreateCommand())
                {
                    command.CommandText = "DROP DATABASE IF EXISTS [SoftUni]; CREATE DATABASE [SoftUni]";
                    command.ExecuteNonQuery();
                }
            };

            using (var conn = new SqlConnection("server=.;database=softuni;integrated security=true;"))
            {
                conn.Open();
                using (var command = conn.CreateCommand())
                {
